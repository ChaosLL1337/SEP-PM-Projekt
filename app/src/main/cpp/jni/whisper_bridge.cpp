...
#include "whisper.h"  // kommt über CMake-Include-Pfade
...
/*
 * Kleiner WAV-Reader für PCM 16-bit, Mono.
 * Gibt Samples als float [-1, 1] zurück und liefert die Sample-Rate (erwartet 16000).
 */
static bool read_wav_mono16(const char* path, std::vector<float>& out, int& sample_rate) {
    ...
    // RIFF
    char riff[4];
    if (fread(riff,1,4,f) != 4 || memcmp(riff,"RIFF",4) != 0) { fclose(f); LOGE("Not RIFF"); return false; }
    uint32_t riff_size; if (!rd32(riff_size)) { fclose(f); return false; }
    char wave[4];
    if (fread(wave,1,4,f) != 4 || memcmp(wave,"WAVE",4) != 0) { fclose(f); LOGE("Not WAVE"); return false; }

    // fmt
    char fmt[4];
    if (fread(fmt,1,4,f) != 4 || memcmp(fmt,"fmt ",4) != 0) { fclose(f); LOGE("No fmt chunk"); return false; }
...
Java_com_example_tut2_WhisperBridge_transcribeWav(
    ...
    // Cleanup-Helper: gibt Strings frei und optional den Whisper-Kontext
    struct Cleaner {
        JNIEnv* env;
        jstring jModelPath;
        jstring jWavPath;
        ...
        const char* lang;
        whisper_context* ctx;
        jstring finish(const std::string& s) {
    ...
    } cleaner{env, jModelPath, jWavPath, jLang, modelPath, wavPath, lang, nullptr};

    // 1) Modell laden
    whisper_context_params cparams = whisper_context_default_params();
    cleaner.ctx = whisper_init_from_file_with_params(modelPath, cparams);
    if (!cleaner.ctx) {
        return cleaner.finish("[whisper] Kontext konnte nicht erstellt werden");
    }

    // 2) WAV lesen (16 kHz, 16-bit, mono)
    std::vector<float> samples;
    int sr = 0;
    if (!read_wav_mono16(wavPath, samples, sr)) {
        return cleaner.finish("[whisper] WAV konnte nicht gelesen werden (erwarte 16-bit/mono)");
    }
    if (sr != 16000) {
        LOGI("WAV sample rate = %d (empfohlen 16000 Hz)", sr);
    }
    ...
    wparams.translate        = false;      // nur transkribieren
    wparams.no_context       = true;       // schneller, kein Kontext
    wparams.single_segment   = false;      // mehrere Segmente okay
    wparams.language         = lang;       // z. B. "de"
    wparams.n_threads        = 4;          // gern erhöhen, je nach Gerät

    // 4) Transkription
    const int rc = whisper_full(cleaner.ctx, wparams, samples.data(), (int)samples.size());
    if (rc != 0) {
        return cleaner.finish("[whisper] Transkription fehlgeschlagen");
    }

    // 5) Segmente einsammeln
    std::string text;
    const int n = whisper_full_n_segments(cleaner.ctx);
    text.reserve(4096);
    for (int i = 0; i < n; ++i) {
        const char* seg = whisper_full_get_segment_text(cleaner.ctx, i);
        if (seg) {
            if (!text.empty()) text += ' ';
            text += seg;
        }
