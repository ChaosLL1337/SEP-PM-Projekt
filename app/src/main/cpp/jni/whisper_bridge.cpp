...
// --- WAV Reader: PCM 16-bit, mono -> float [-1..1] ---
static bool read_wav_mono16(const char* path, std::vector<float>& out, int& sample_rate) {
    ...
    auto rd32 = [&](uint32_t& v){ unsigned char b[4]; if (fread(b,1,4,f)!=4) return false;
        v=(uint32_t)b[0]|((uint32_t)b[1]<<8)|((uint32_t)b[2]<<16)|((uint32_t)b[3]<<24); return true; };
    auto rd16 = [&](uint16_t& v){ unsigned char b[2]; if (fread(b,1,2,f)!=2) return false;
        v=(uint16_t)b[0]|((uint16_t)b[1]<<8); return true; };
    auto skip=[&](size_t n){ return fseek(f,(long)n,SEEK_CUR)==0; };

    // RIFF/WAVE
    char riff[4]; if (fread(riff,1,4,f)!=4 || memcmp(riff,"RIFF",4)!=0){ fclose(f); return false; }
    uint32_t riff_size; if (!rd32(riff_size)) { fclose(f); return false; }
    char wave[4]; if (fread(wave,1,4,f)!=4 || memcmp(wave,"WAVE",4)!=0){ fclose(f); return false; }

    // fmt
    char fmt[4]; if (fread(fmt,1,4,f)!=4 || memcmp(fmt,"fmt ",4)!=0){ fclose(f); return false; }
    ...
    // nur PCM, mono, 16-bit
    if (audio_format!=1 || num_channels!=1 || bitsPerSample!=16) { fclose(f); return false; }
    ...
    fclose(f);
    if (read!=n_samples) return false;
...
static void json_escape_append(const char* s, std::string& out) {
    ...
    for (const char* p=s; *p; ++p) {
        ...
        switch (c) {
            ...
            case '\r':
            case '\t':
                // auslassen / normalisieren
                break;
            default:
                if (c < 0x20) { /* ignore control */ }
...
// -----------------------------------------------------------------------------
// JNI #1: Plain-Text (Segmente zusammengefügt)
// -----------------------------------------------------------------------------
extern "C"
...
Java_com_example_tut2_WhisperBridge_transcribeWav(
    ...
    wparams.print_timestamps = false;   // <-- reiner Text
    ...
    if (out.empty()) out = "[whisper] Kein Text erkannt";
...
// -----------------------------------------------------------------------------
// JNI #2: JSON-Segmente [{ "t0_ms":<long>, "t1_ms":<long>, "text":"..." }, ...]
// Hinweis: whisper_full_get_segment_t{0,1} liefern 10-ms-Ticks -> *10 = ms
// -----------------------------------------------------------------------------
extern "C"
JNIEXPORT jstring JNICALL
Java_com_example_tut2_WhisperBridge_transcribeWavSegments(
    ...
    wparams.print_realtime   = false;
    wparams.print_progress   = false;
    wparams.print_timestamps = true;    // <-- wir wollen Segmentzeiten
    wparams.print_special    = false;
    wparams.translate        = false;
    wparams.no_context       = true;
    wparams.single_segment   = false;
    ...
    for (int i=0;i<n;++i) {
        const char* seg = whisper_full_get_segment_text(ctx,i);
        const int t0_cs = whisper_full_get_segment_t0(ctx,i); // 10 ms steps
        const int t1_cs = whisper_full_get_segment_t1(ctx,i);

        const long long t0_ms = (long long)t0_cs * 10LL;
        const long long t1_ms = (long long)t1_cs * 10LL;
